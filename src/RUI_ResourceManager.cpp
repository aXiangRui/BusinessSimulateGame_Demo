...
SDL_Texture* ResourceManager::FindTexture(const std::string& path)
{
    auto it = TexturePool.find(path);
    return it != TexturePool.end() ? it->second : nullptr;
}

Mix_Music* ResourceManager::FindMusic(const std::string& path)
{
    auto it = MusicPool.find(path);
    return it != MusicPool.end() ? it->second : nullptr;
}

Mix_Chunk* ResourceManager::FindAudio(const std::string& path)
{
...
void ResourceManager::load(SDL_Renderer* Renderer)
{
    ...
    SDL_Log("Resource scan finished. textures=%d audio=%d music=%d",
        (int)TexturePool.size(), (int)AudioPool.size(), (int)MusicPool.size());
}
